...
export default function VisionSection() {
  return (
    <section className="relative w-full py-24 md:py-32 px-6 lg:px-12 xl:px-20 bg-[#0a131f]">
      {/* Decorative lines */}
      <div className="absolute left-0 top-1/2 -translate-y-1/2 w-16 md:w-24 lg:w-32 h-px bg-gradient-to-r from-transparent to-[#ce9d22]/30" />
      <div className="absolute right-0 top-1/2 -translate-y-1/2 w-16 md:w-24 lg:w-32 h-px bg-gradient-to-l from-transparent to-[#ce9d22]/30" />

      <div className="flex flex-col max-w-5xl mx-auto items-center gap-6 md:gap-8">
        {/* Section Label */}
        ...
        {/* CTA Button */}
        <button className="mt-4 h-14 px-10 bg-[#EFBF04] hover:bg-[#d4a84a] rounded-md flex items-center gap-2 transition-colors">
          <span className="text-[#0D1B2A] text-base font-semibold">Enter Klock Aviation</span>
          <ArrowRightIcon className="w-5 h-5 text-[#0D1B2A]" />
        </button>
      </div>
    </section>
  );
